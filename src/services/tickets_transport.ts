...
export const getOneRole = async (idboleto: number | any) => {
  try {
    //consultas a la base de datos van aca
    const role = await tickets_transport.findOne({ where: { idboleto } }); // Busca el proyecto con título 'Mi Título'
    if (role === null) {
      console.log("No encontrado");
      return {
...
export const findRoleByName = async (name: string) => {
  try {
    //consultas a la base de datos van aca
    const role = await tickets_transport.findOne({
      where: {
        name: name,
      },
    });
